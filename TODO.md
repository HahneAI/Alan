...
The `ai_conversations` row is created on first message send, not on "New Session" click. The Edge Function checks whether `conversationId` is `null` — if so, it inserts the conversation row first, then proceeds normally. The client receives the new `conversationId` in the response and holds it for subsequent messages.

### Streaming
The React client handles streaming via `fetch` with `ReadableStream`. Tokens are rendered progressively as they arrive. Do not use the Vercel AI SDK.

### Auto-title generation
Format: `[Persona/Goal target] — [4-word max concise topic summary of first message]`. Generated by a lightweight Claude call after the first exchange — not the full Alan Brain call. Fires asynchronously; the client updates the conversation title in the left rail when the title response arrives.

### Error and retry failure modes
Three failure points, all surface a retry button inline in the chat thread:
1. **Anthropic API down** — catch non-2xx from the Anthropic SDK; surface immediately.
2. **Edge Function timeout** — Supabase Edge Functions have a 150 s wall-clock limit; if the streaming connection drops before completion, surface the retry.
3. **Supabase write failure** — attempt silent re-persist once before surfacing the error to the user.
...
### AI routing notes (Phase 8)
- **Coaching summary** — routes through the Alan Brain (full system prompt + dynamic user context injected). This is a coached, personalised paragraph, not a data dump.
- **Raw metrics processing** (Yoodli/Deepgram/Whisper normalisation) — a separate utility call with no Alan Brain involvement; pure data transformation into the canonical report shape.
...
### AI routing notes (Phase 9)
- **Live real-time metrics during Presentation Mode** (pacing gauge, filler count, tone) — compact direct Claude calls, no Alan Brain. Speed matters more than persona here; keep the prompt minimal.
- **Post-presentation class feedback card** — routes through Alan Brain (full system prompt + user context). This is the public-facing coached summary the whole class sees; Alan's voice must be consistent.
- **Full session recap coaching summaries** — also route through Alan Brain, one call per presenter, fired asynchronously after `process-session-recording` completes.
